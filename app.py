...
.chat-container { display:flex; flex-direction:column; gap:12px; padding:16px 0; max-height:420px; overflow-y:auto; }
.bubble-user { align-self:flex-end; background:#1e40af; color:white; padding:10px 16px; border-radius:18px 18px 4px 18px; max-width:75%; font-size:14px; line-height:1.5; }
.bubble-bot { align-self:flex-start; background:#1e293b; color:#e2e8f0; padding:10px 16px; border-radius:18px 18px 18px 4px; max-width:75%; font-size:14px; line-height:1.5; border:1px solid #334155; }
...
.tag-pending { background:#312e81; color:#a5b4fc; }
.metric-card { background:#1e293b; border:1px solid #334155; border-radius:12px; padding:16px 20px; text-align:center; }
.metric-num { font-size:28px; font-weight:700; color:#60a5fa; }
.metric-lbl { font-size:12px; color:#94a3b8; margin-top:4px; }
.grievance-card { background:#1e293b; border:1px solid #334155; border-radius:10px; padding:14px 18px; margin-bottom:10px; }
.grievance-id { font-size:11px; color:#64748b; }
.grievance-text { font-size:14px; color:#e2e8f0; margin:6px 0; }
.grievance-meta { font-size:12px; color:#94a3b8; }
.admin-note { background:#0f2744; border-left:3px solid #3b82f6; padding:8px 12px; border-radius:0 6px 6px 0; font-size:13px; color:#93c5fd; margin-top:8px; }
.pill { display:inline-block; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; letter-spacing:0.5px; }
.pill-pending { background:#312e81; color:#a5b4fc; }
.pill-progress { background:#1c3a2a; color:#6ee7b7; }
.pill-resolved { background:#14532d; color:#86efac; }
.pill-escalated { background:#7f1d1d; color:#fca5a5; }
.pill-closed { background:#1e293b; color:#64748b; border:1px solid #334155; }
.level1-banner { background:linear-gradient(135deg,#1e3a5f,#1e293b); border:1px solid #3b82f6; border-radius:10px; padding:12px 18px; margin-bottom:16px; color:#93c5fd; font-size:13px; }
.level2-banner { background:linear-gradient(135deg,#450a0a,#1e293b); border:1px solid #ef4444; border-radius:10px; padding:12px 18px; margin-bottom:16px; color:#fca5a5; font-size:13px; }
.flow-container { display:flex; align-items:center; justify-content:center; gap:0; flex-wrap:wrap; margin:24px 0; }
.flow-node { background:#1e293b; border:2px solid #334155; border-radius:12px; padding:16px 20px; text-align:center; min-width:130px; }
.flow-node .icon { font-size:24px; }
.flow-node .label { font-size:13px; font-weight:600; color:#e2e8f0; margin-top:4px; }
.flow-node .sublabel { font-size:11px; color:#64748b; margin-top:2px; }
.flow-arrow { font-size:22px; color:#475569; padding:0 6px; }
.flow-node-pending { border-color:#6366f1; }
.flow-node-progress { border-color:#22d3ee; }
.flow-node-escalated { border-color:#ef4444; }
.flow-node-resolved { border-color:#22c55e; }
.flow-node-closed { border-color:#64748b; }
.flow-node-auto { border-color:#f97316; background:#1c1208; }
</style>
...
def page_tracking(grievances, is_admin=False):
    ...
    # ── Section 1: Lifecycle Flowchart ────────────────────────────────────────
    st.markdown("#### Grievance Lifecycle Flow")
    st.markdown(render_flowchart(), unsafe_allow_html=True)

    col_info1, col_info2, col_info3 = st.columns(3)
    col_info1.info("**Level 1 (HR Manager):** Pending → In Progress → Resolve (Low/Medium) or Escalate (High)")
    col_info2.warning("**Level 2 (Dept Head):** Final authority → Resolve or Close")
    col_info3.error(f"**Auto-Escalation:** No action within **{ESCALATION_HOURS}h** → auto-escalated ⚡")
...
def page_employee():
    ...
    with tab1:
        st.markdown(f"### Welcome, {user['name'].strip()}")
...
def page_senior_admin():
    st.markdown("### 🟥 Department Head Panel")
...
def sidebar():
    with st.sidebar:
        st.markdown("### ⚖️ Grievance System")
        st.caption("AI-Based Redressal & Escalation")
        st.divider()
        if "user" not in st.session_state:
            page=st.radio("",["🔐 Login","📝 Register"],label_visibility="collapsed")
            st.divider()
            st.caption("**Escalation Structure**")
            st.markdown("🟦 **Level 1** → HR Manager\n\n🟥 **Level 2** → Department Head")
            return page
        u=st.session_state.user
        st.text(u["name"].strip())
        st.caption(ROLE_LABELS.get(u["role"],u["role"]))
        st.caption(u["email"])
        st.divider()
        ...
        st.divider()
        st.caption("**Workflow**")
        st.markdown("1️⃣ Employee submits\n\n2️⃣ 🟦 HR Manager reviews\n\n3️⃣ HR escalates if needed\n\n4️⃣ 🟥 Dept Head resolves")
        st.divider()
        st.caption("**Auto-Escalation**")
        st.markdown(f"⚡ Unresolved → Level 2 after **{ESCALATION_HOURS}h**")
        st.divider()
        st.caption("**Priority**")
